...
# ------- Preparing the map --------
map <- readOGR(dsn = "gadm41_IRL_shp", layer = "gadm41_IRL_1")
df_map = st_as_sf(map)
df_map = df_map %>% mutate(NAME_1 = as.character(NAME_1),
...
df_counters = t(posteriors[['y_counter']]) %>% 
  ...
  summarise(across(V1:V6500, ~sum(.)))


df_factuals = df_jags %>% 
  group_by(county) %>% 
  summarise(notices = sum(notices))


df_count_vs_factual = left_join(df_counters, df_factuals) %>% 
  mutate(across(V1:V6500, ~. - notices))

df_quantiles = as.data.frame(t(apply(df_count_vs_factual %>% select(V1:V6500), 1, function(x) quantile(x, prob = c(0.025, 0.5, 0.975)))))


df_map_saved_lives = cbind(df_quantiles,df_map) %>% 
  st_as_sf()

min_saved_lives = min(st_drop_geometry(df_map_saved_lives)[,1:3])
max_saved_lives = max(st_drop_geometry(df_map_saved_lives)[,1:3])

p_low = df_map_saved_lives %>% 
  rename(`Number of\nsaved lives` = `2.5%`) %>% 
  ggplot() + geom_sf(aes(fill = `Number of\nsaved lives`), lwd = 0.1) +
  scale_fill_gradient2(
    midpoint = 0, limits = c(min_saved_lives, max_saved_lives),low = "red", mid = "white", high = "green4"
  ) +
